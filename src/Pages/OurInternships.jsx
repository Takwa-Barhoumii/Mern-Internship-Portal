...
const OurInternships = () => {
  ...
  useEffect(() => {
    setIsloading(true);
    fetch("http://localhost:5000/all-internships")
      .then((res) => res.json())
      .then((data) => {
        setInternships(data);
        setIsloading(false);
  ...
  const filteredItems = internships.filter(
    (internship) =>
      internship.internshipTitle.toLowerCase().indexOf(query.toLowerCase()) !==
      -1
  );
  ...
  const filteredData = (internships, selected, query) => {
    ...
    if (selected) {
      filteredInternships = filteredInternships.filter(
        ({ internshipLocation, payment, postingDate, employmentType }) =>
